...
2. Распакуйте и переместите `MacDashboard.app` в `/Applications` (или `~/Applications`).
3. **Первый запуск — разовый шаг с Gatekeeper.** Приложение подписано ad-hoc, но
   ...
   **Или через Системные настройки (без терминала):**
   ...
   > В macOS 15 (Sequoia) и новее старый трюк «правый клик → Открыть» больше не обходит Gatekeeper — нужно именно через Системные настройки.
