...
- История (занятость диска, циклы батареи) копится в
  ...
  (совместим со старым форматом).
- Обнаруживает недавние креш-логи (окно 7 дней), группирует и сворачивает их,
  отдельно отмечая падения самого приложения и системные паники.
- Температура SoC и внутренних дисков — через приватный HID-API сенсоров Apple Silicon.
...
Нужен **Xcode 26 (Tahoe SDK)** с выбранными командными инструментами — приложение
использует SwiftUI API (`.onGeometryChange`), который есть только в этом SDK, и
более старый Xcode сборку не осилит. Это требование только к сборке: готовое
приложение по-прежнему работает на macOS 14 (Sonoma) и новее.
...
Необязательно, один раз на Mac: `tools/signing/make-identity.sh` создаёт в связке
ключей «login» локальный самоподписанный сертификат для подписи, и все следующие
`./build_app.sh` подписывают приложение им. Тогда macOS узнаёт каждую новую сборку
как то же самое приложение, и полный доступ к диску и другие разрешения переживают
пересборку. Без него сборка подписывается ad-hoc (сборка выводит предупреждение), и
после пересборки с изменённым кодом macOS снова спрашивает эти разрешения. Скрипт
может один раз показать окно связки ключей: введите пароль входа в систему и нажмите
«Разрешить всегда».
...
- `Tests/Fixtures/` — сохранённые выводы команд, на которых проверяются парсеры (см. README в этой папке).
- `tools/visual/run.sh` — визуальная проверка: снимает реальные окна приложения и сравнивает их с эталонными скриншотами (инструмент разработки).
- `build_app.sh` — сборка для Apple Silicon (arm64) + упаковка .app + подпись (локальным сертификатом, если он есть, иначе ad-hoc).
- `tools/signing/make-identity.sh` — разовое создание этого локального сертификата.
